...
**Принятые вводные (детали в §7 Этап 0):** домен `palisoft.ru` (единственный) · позиционирование «мы» (ИП, Organization + Person) · рынок — весь мир, английский приоритетный · цены в USD (250 / 1000 / 6000) · индексация целиком под флагом `ALLOW_INDEXING_BOTS`, по умолчанию **выключена**.
...
Не хватает: запрета служебных путей (`/_next/`, `/api/`), явного `Host` (устаревшая директива Яндекса, но безвредна), и — по желанию владельца — правил для AI-краулеров (`GPTBot`, `ClaudeBot`, `PerplexityBot`, `CCBot`). Для портфолио разработчика я бы **разрешил** их: цитирование в AI-ответах сейчас реальный канал входящих обращений.
...
| Q5 | Индексация | **Всё под флагом `ALLOW_INDEXING_BOTS=true`. По умолчанию — выключено** | Глобальный kill-switch: `robots.txt` + `<meta robots>`; включается одним флагом на запуске |

**Стек для `/pricing` (по Q4):** `supabase`, `nextjs`, `nestjs`, `react`, `threejs`, `prisma`, `clickhouse`, `duckdb`, `kafka`, `redis`, `postgres`.

> ⚠️ **Важно по Q5.** Формулировку «пока по умолчанию всё отключено» я реализую как **глобальный запрет индексации всего сайта** (не только AI-ботов): `robots.txt` отдаёт `Disallow: /` и на каждой странице ставится `<meta name="robots" content="noindex, nofollow">`. При `ALLOW_INDEXING_BOTS=true` — открывается всё, включая `GPTBot` / `ClaudeBot` / `PerplexityBot` / `Google-Extended`.
...
>
> **Двойное предупреждение о механике флага:** страницы у нас SSG, значит значение переменной **впекается в HTML на этапе сборки**. Смена `ALLOW_INDEXING_BOTS` требует **пересборки и передеплоя**, а не просто рестарта. Забыть снять флаг на запуске = сайт остаётся невидимым для поиска при полностью выполненном плане. Поэтому: пункт «снять флаг» вынесен в §11 отдельным блокирующим шагом, а в Этап 1 добавлена проверка, падающая на билде, если продакшен собирается с `noindex`.
...
- [ ] Добавить `NEXT_PUBLIC_SITE_URL=https://palisoft.ru` и `ALLOW_INDEXING_BOTS=false` в env продакшена и в `.env.example`.
...
Защита от «забыли снять флаг» — падающая проверка в `next.config.ts`:

```ts
// next.config.ts
if (
  process.env.NODE_ENV === 'production' &&
  process.env.VERCEL_ENV === 'production' &&      // или свой признак прод-деплоя
  process.env.ALLOW_INDEXING_BOTS !== 'true' &&
  process.env.ALLOW_NOINDEX_BUILD !== 'true'      // осознанный обход
) {
  throw new Error(
...
- [ ] Пока сайт не готов — деплоить с `ALLOW_NOINDEX_BUILD=true`
- [ ] На запуске — `ALLOW_INDEXING_BOTS=true`, убрать `ALLOW_NOINDEX_BUILD`, **пересобрать и передеплоить**
...
**Критерий приёмки этапа 1** (собрать с `ALLOW_INDEXING_BOTS=true`, иначе проверять нечего):

```bash
ALLOW_INDEXING_BOTS=true pnpm build
...
> ⛔ **Всё в этом этапе выполняется только после `ALLOW_INDEXING_BOTS=true` (Q5).** Подтверждать домен и отправлять sitemap при активном `noindex` бессмысленно: панели просто зафиксируют «страница исключена тегом noindex», и это придётся расчищать. Единственное исключение — аналитику можно подключить раньше.
...
Этап 2 (OG + JSON-LD)  +  Этап 3.0 (цены и стек — быстро и заметно)
   ↓
🚩 СНЯТЬ ФЛАГ: ALLOW_INDEXING_BOTS=true → пересборка → передеплой
   ↓
